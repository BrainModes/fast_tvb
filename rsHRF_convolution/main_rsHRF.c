...
void *run_simulation(void *arg)
{
    ...
    for (ts = 0; ts < time_steps; ts++) 
    {
        ...
        if (ts >= divRoundClosest(10000/model_dt, 1) && ts <= (FIC_time_steps) && ts % divRoundClosest(10000/model_dt, 1) == 0) 
        {
            ...
            {
                meanFR[i_node_vec_local]           = meanFR[i_node_vec_local] / i_meanfr;
                meanFR_INH[i_node_vec_local]       = meanFR_INH[i_node_vec_local] / i_meanfr;
                mean_mean_FR        += meanFR[i_node_vec_local];
                mean_mean_FR_INH    += meanFR_INH[i_node_vec_local];
                i_node_vec_local ++;
            }
 
            mean_mean_FR            /= (end_nodes_mt_glob - start_nodes_mt);
            mean_mean_FR_INH        /= (end_nodes_mt_glob - start_nodes_mt);


            for(; i_node_vec < end_nodes_mt; i_node_vec++)
            {
                meanFR[i_node_vec_local]           = meanFR[i_node_vec_local] / i_meanfr;
                meanFR_INH[i_node_vec_local]       = meanFR_INH[i_node_vec_local] / i_meanfr;
                i_node_vec_local ++;
            ...
            for (i_node_vec = start_nodes_mt; i_node_vec < end_nodes_mt_glob; i_node_vec++)
            {
                tmpvar              = meanFR[i_node_vec_local] - mean_mean_FR;
            ...
            printf("time (s): %d\t\tmean+/-std firing rate exc. pops.: %.2f +/- %.2f\n", divRoundClosest(ts*model_dt/1000, 1), mean_mean_FR, std_FR);
            ...
            */
            ...
            for (i_node_vec = start_nodes_mt; i_node_vec < end_nodes_mt; i_node_vec++)
            {
                float pre  = meanFR_INH[i_node_vec_local];
                float post = meanFR[i_node_vec_local];
                J_i_local[i_node_vec_local] +=  isp_eta * (pre * post - isp_r0 * pre);
                J_i_local[i_node_vec_local] = J_i_local[i_node_vec_local] > 0 ? J_i_local[i_node_vec_local] : 0; // make sure that weight is always >=0
                J_i[i_node_vec] = J_i_local[i_node_vec_local];                              // copying to the global value at each cycle
                i_node_vec_local ++;
            }
            ...
            for (i_node_vec = start_nodes_mt; i_node_vec < end_nodes_mt; i_node_vec++)
            {
                meanFR[i_node_vec_local]           = 0.0;
                meanFR_INH[i_node_vec_local]       = 0.0;
                i_node_vec_local ++;
            }
        }
